...
def get_features(document, candidate, candidate_list, isKeyword):
    ...
    features['part_of_speech'] = ' '.join(
        [pos for word, pos in nltk.pos_tag(nltk.word_tokenize(candidate))])

    position_list = [m.start()/float(len(document))
                     for m in re.finditer(re.escape(candidate), document, flags=re.IGNORECASE)]
    ...
    if len(position_list):
        avg_position = sum([pos*float(len(document))
                            for pos in position_list])/float(len(position_list))
        ss = 0
    ...
    features['frequency'] = len(position_list) / \
        float(len(set(candidate_list)))  # check
    ...
    if doc.endswith('txt'):
        ...
        for team in os.listdir(team_path):
            ...
            if team.startswith('team'):
                ...
                keywords = open(team_path + team + '/' +
                                doc_name + '.key', "r").read()
                # keywords = unicode(keywords + team + '/' + doc_name + '.key', errors='replace')
                keywords = keywords + team + '/' + doc_name + '.key'

                # Take list of Keywords
                keyword_list.extend([line.split(':')[1].lower().strip()
                                     for line in keywords.splitlines() if ':' in line])

        feature_list.extend(
            [(get_features(document, key, candidates, 1), 1) for key in set(keyword_list)])
        feature_list.extend([(get_features(document, key, candidates, 0), 0)
                             for key in candidates if key not in set(keyword_list)])
...
print('all indicies of keywords training', len(
    [x for x, y in training_set if y == 1]))

testing_set = []
testing_set.extend(good_list_test)
testing_set.extend(bad_list_test)
print('all indicies of keywords testing', len(
    [x for x, y in testing_set if y == 1]))
...
print("LinearSVC_classifier algorithm accuracy : ",
